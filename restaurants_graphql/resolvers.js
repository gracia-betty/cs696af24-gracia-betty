...
const resolvers = {
  Query: {
    ...
    async restaurants(_, { borough, cuisine }) {
      ...
      return await collection.find(query).toArray(); // Return matching restaurants
    },
  },

  Mutation: {
    async createRestaurant(_, { restaurant_id, name, borough, cuisine }) {
      let collection = await db.collection("restaurants");
    ...
    async deleteRestaurant(_, { restaurant_id }) {
      let collection = await db.collection("restaurants");
